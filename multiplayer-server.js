...
export class MultiplayerServer {
  ...
  generateWorld() {
    ...
    this.state.walls = this.generateWalls();
  ...
  generateWalls() {
    const { GRID_CELL_SIZE, WALL_THICKNESS, WORLD_SIZE, circleIntersectsRect } = this.deps;
    const result = [];
    const wallCount = 3 + Math.floor(this.random() * 3);
    for (let i = 0; i < wallCount; i += 1) {
      const orientation = this.random() < 0.5 ? "horizontal" : "vertical";
      const lengthCells = 1 + Math.floor(this.random() * 3);
      const width =
        orientation === "horizontal"
          ? lengthCells * GRID_CELL_SIZE
          : WALL_THICKNESS;
      const height =
        orientation === "horizontal"
          ? WALL_THICKNESS
          : lengthCells * GRID_CELL_SIZE;
      const x = this.random() * (WORLD_SIZE - width);
      const y = this.random() * (WORLD_SIZE - height);
      const centerX = x + width / 2;
      const centerY = y + height / 2;
      const distanceToCenter = Math.hypot(centerX - WORLD_SIZE / 2, centerY - WORLD_SIZE / 2);
      if (distanceToCenter < 80) {
        continue;
      }
      const intersectsPlayer = this.state.players.some((player) =>
        circleIntersectsRect(player.x, player.y, player.size / 2 + 6, {
          x,
          y,
          width,
          height
  ...
  generateMines() {
    const { MAX_MINES, MINE_SIZE, WORLD_SIZE, MINE_MIN_DISTANCE } = this.deps;
    const minesResult = [];
    const mineCount = Math.floor(this.random() * (MAX_MINES + 1));
    const radius = MINE_SIZE / 2;
    for (let attempt = 0; attempt < 200 && minesResult.length < mineCount; attempt += 1) {
      const x = radius + this.random() * (WORLD_SIZE - radius * 2);
      const y = radius + this.random() * (WORLD_SIZE - radius * 2);
      if (this.isCircleBlocked(x, y, radius + MINE_MIN_DISTANCE, minesResult)) {
        continue;
      }
      const intersectsPlayer = this.state.players.some((player) =>
        Math.hypot(player.x - x, player.y - y) < player.size / 2 + radius + MINE_MIN_DISTANCE
      );
      if (intersectsPlayer) {
        continue;
      }
  ...
  isCircleBlocked(x, y, radius, minesList = this.state.mines) {
    const { WORLD_SIZE, circleIntersectsAnyWall } = this.deps;
    if (
      x - radius < 0 ||
      y - radius < 0 ||
      x + radius > WORLD_SIZE ||
    ...
    if (circleIntersectsAnyWall(x, y, radius, this.state.walls)) {
      return true;
    }
    return (minesList || []).some((mine) => Math.hypot(mine.x - x, mine.y - y) < mine.size / 2 + radius);
  }
  ...
  spawnFish(forceNormal = false) {
    const { WORLD_SIZE, GOLDEN_FISH_CHANCE, getFishTimeLimitForFish } = this.deps;
    const margin = 30;
    ...
    for (let attempt = 0; attempt < 200; attempt += 1) {
      ...
      if (this.isCircleBlocked(x, y, fishState.size / 2 + 4, this.state.mines)) {
        continue;
      }
      const closeToPlayer = this.state.players.some(
        (player) => player.alive && Math.hypot(player.x - x, player.y - y) < player.size
      );
      if (closeToPlayer) {
        continue;
      }
